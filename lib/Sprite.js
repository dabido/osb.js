...
/**
 * Adds a osu! trigger. Using the existing loop functionality
 * @param  {int} duration    Trigger duration
 * @param  {string} triggerType Trigger type
 * @return {Sprite}             The sprite itself
 */
Sprite.prototype.startTrigger = function (duration, triggerType) {
	if (!this.inLoop) {
		this.inLoop = true;
		this.loopDuration = 0;
		this.loopcount = 0;
		this.chain.push("_T," + triggerType + "," + this.ms + "," + (this.ms + duration));
	}
 ...
 * @return {Sprite} the sprite itself
 */
Sprite.prototype.endTrigger = function() {
	this.inLoop = false;
	this.ms += this.loopDuration;
	return this;
};